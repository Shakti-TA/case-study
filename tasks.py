...
def start_tracker_ui(c, platform=PLATFORM, env=DEV_ENV, port=8082):
    ...
    with py_env(c, env_name):
        # # To use sqlite as the backend storage and store every run as in
        # # a databse, uncomment out the following:
        # c.run(
        #     f"""mlflow server --port {port} --default-artifact-root "{HERE}"/mlruns """
        #     f"""--backend-store-uri sqlite:///"{HERE}"/mlruns/mlflow.sqlite """
        #     f"--workers 1"
        # )
        c.run(
            f"""mlflow server --port {port} --default-artifact-root "{HERE}"/mlruns """
            f"""--backend-store-uri "{HERE}"/mlruns/ """
            f"--workers 1"
        )
...
# My Tasks
# --------------
@task(name="run-production-main")
def run_production_main_script(c, env=DEV_ENV):
    """My task to run the main script of the production (cli.py)."""

    # Get the conda environment's name
    env_name = _get_env_name(env)

    # Define the production main file path
    production_main_path = op.join(HERE, "production", "cli.py")

    # Run the cli command
    with py_env(c, env_name):
        c.run(f"python {production_main_path} job run")


@task(
    name="get-codebase-complexity",
    help={
        "metrics_type": (
            "Any one of the complexity metrics as follows: "
            ...
            "DEFAULT is `cc`: Cyclomatic Complexity"

        )
    }
)
def get_codebase_complexity(c, env=DEV_ENV, metrics_type="cc"):
    """
    My task to get the complexity of the production codebase.
    [based on the four metrics available, viz.
    'Cyclomatic Complexity', 'Raw Metrics', 'Maintainability Index',
    'Halstead Complexity Metric']

    NOTE: The configuration for ``cc`` and ``mi`` is defined in the `setup.cfg`
    ...
    # Define the production main file path
    production_main_path = op.join(HERE, "production")

    # Run the cli command
    with py_env(c, env_name):
        base_command = f"radon {metrics_type} {production_main_path}"
        if metrics_type == "raw":
            base_command += " --summary"
...
_create_task_collection(
    "domytask",
    run_production_main_script,
    get_codebase_complexity
)

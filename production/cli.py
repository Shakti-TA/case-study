...
def _run_job(cli_ctx, job_id, num_workers, num_threads_per_worker):
    ...
    remote_server_uri = "http://localhost:8082"
    mlflow.set_tracking_uri(remote_server_uri)
    mlflow.set_registry_uri(remote_server_uri)

    print(mlflow.tracking.get_tracking_uri())

    # Set the experiment and get the experiment ID
    ...
    with mlflow.start_run(
        ...
        run_name="PARENT-CLI-RUN",
        description="This is the main CLI run function that runs all the tasks."
    ) as cli_run:  # noqa
        mlflow.log_param("parent", "yes")
        mlflow.log_param("number_of_workers", num_workers)
        mlflow.log_param("number_of_threads_per_worker", num_threads_per_worker)
        mlflow.log_param("job_catalog", job_catalog)
